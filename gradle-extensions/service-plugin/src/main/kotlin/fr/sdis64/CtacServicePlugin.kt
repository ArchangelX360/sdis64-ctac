...
class CtacServicePlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        service.environments.environmentsByName.whenObjectAdded(closureOf<CtacEnvironment>{
            project.configuringEnvironmentTasks(service.name, this)
        })
    }
}

private fun Project.configuringEnvironmentTasks(serviceName: Provider<String>, env: CtacEnvironment) {
    val envName = env.name.get()
    val capitalizedEnvName = envName.replaceFirstChar { it.uppercase() }

    val dockerTag = serviceName.map { service -> project.buildCtacDockerImageTag(service, envName) }
